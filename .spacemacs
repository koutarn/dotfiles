;;このファイルは起動時にSpacemacsによってロードされます。
;;ホームディレクトリに保存する必要があります。

;;layerの設定
(defun dotspacemacs/layers ()
;;変数を変更する以外に、この関数にはユーザーコードを入れないでください。
  (setq-default
   ;; 基本レイヤの設定 `spacemacs-base'or `spacemacs'. (default 'spacemacs)
   dotspacemacs-distribution 'spacemacs

   ;; lazy install設定
   ;; (default 'unused)
   dotspacemacs-enable-lazy-installation 'unused
   ;; nil以外の場合、Spacemacsはインストールの前に確認を求めます。
   ...
   ;;インストールおよびロードされないパッケージのリスト。
   dotspacemacs-excluded-packages '()

   ;;パッケージをインストールするときのSpacemacsの動作を定義します。
   ;;可能な値は `used-only '、` used-but-keep-unused'、そして `all 'です。
   ;;used-only 'は明示的に使われているパッケージだけをインストールし、それをアンインストールする
   ;;未使用のパッケージとその未使用の依存関係
   ;;used-but-keep-unused 'は使用されたパッケージだけをインストールしますがアンインストールはしません
   ;;未使用になった場合`all 'は、によってサポートされている* all *パッケージをインストールします。
   ;; Spacemacsとそれらをアンインストールしないでください。 （デフォルトは `used-only 'です）
   dotspacemacs-install-packages 'used-only))

(defun dotspacemacs/init ()
  "この関数は、Spacemacsの初期化の開始時に呼び出されます
   レイヤー構成の前に。変数を変更する以外に、そこにユーザーコードを入れないでください。"
